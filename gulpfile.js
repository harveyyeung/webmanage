...
gulp.task('copyfonts', function (done) {
    gulp.src(['src/css/fonts/**/*.{ttf,woff,eof,svg}']).pipe(gulp.dest('dist/fonts')).on('end', done);
});
//将图片拷贝到目标目录
gulp.task('copyplug', function (done) {
    gulp.src(['src/js/plug/**/*']).pipe(gulp.dest('dist/js')).on('end', done);
});


//压缩合并css, css中既有自己写的.less, 也有引入第三方库的.css
gulp.task('lessmin', function (done) {
    gulp.src(['src/css/main.less', 'src/css/common/*.css'])
        .pipe(less())
